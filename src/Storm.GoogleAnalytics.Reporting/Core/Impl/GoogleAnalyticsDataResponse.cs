...
namespace Storm.GoogleAnalytics.Reporting.Core.Impl
{
    public sealed class GoogleAnalyticsDataResponse : IGoogleAnalyticsDataResponse
    {
        ...
        public string AsJson(bool prettyPrinted = false)
        {
            try
            {
                return AsDataTable() != null ? JsonConvert.SerializeObject(AsDataTable(), prettyPrinted ? Formatting.Indented : Formatting.None, new DataTableConverter()) : null;
            }
            catch (Exception ex)
            {
        ...
        public IEnumerable<TEntity> ToObject<TEntity>()
        {
            try
            {
                return AsDataTable() != null ? Mapper.Map<IDataReader, IEnumerable<TEntity>>(AsDataTable().CreateDataReader()) : Enumerable.Empty<TEntity>();
            }
            catch (Exception ex)
            {
